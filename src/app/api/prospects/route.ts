...
import { isSyntheticHandoffProspect, SYNTHETIC_HANDOFF_SOURCE } from "@/lib/synthetic-handoff";
...
export async function PATCH(request: Request) {
  try {
    ...
    if (updates.source === SYNTHETIC_HANDOFF_SOURCE || (current.source === SYNTHETIC_HANDOFF_SOURCE && Object.keys(updates).some((key) => key !== "notes"))) {
      return NextResponse.json({ error: "Synthetic handoff records permit notes updates only" }, { status: 400 });
    }
...
export async function POST(request: Request) {
  try {
    ...
    if (source === SYNTHETIC_HANDOFF_SOURCE && !isSyntheticHandoffProspect(body)) {
      return NextResponse.json({ error: "Synthetic QA requires the reserved TEST ONLY name, example.invalid email, and no phone" }, { status: 400 });
    }
